...
def generate_subkeys(key: int) -> dict:
    ...
    """the 128-bit key K is used as KL 
    and KR is 0"""
    KL = key
    KR = 0

    """The 128-bit variables KA and KB are
    generated from KL and KR as follows.
    Note that KB is used only if the length
    of the secret key is 192 or 256 bits.
    ...
    #128-bit, but KB is here for completeness
    KB = (D1 << 64) | D2 

    """64-bit subkeys are generated by
    rotating KL, KR, KA, and KB and
...
def generate_subkeys_192(key: int) -> dict:
    ...
    """the leftmost 128-bits of key K are
    used as KL and the concatenation of
    the rightmost 64-bits of K and the
    complement of the rightmost 64-bits of
    K are used as KR"""
    KL = key >> 64
...
def generate_subkeys_256(key: int) -> dict:
    ...
    """the leftmost 128-bits of key K are
    used as KL and the rightmost 128-bits
    of K are used as KR"""
    KL = key >> 128
    KR = key & MASK128
